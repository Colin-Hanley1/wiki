...
- **Multi-step transition probabilities via matrix powers:** Computing P^n to find the probability of transitioning between states in exactly n steps.
- **Ehrenfest chain (N=3):** A classic model of diffusion between two chambers. Analyzing the transition matrix and computing multi-step probabilities for 3 particles.
...
- **Stationary distributions:** Solving the system pi P = pi subject to sum(pi) = 1. Finding the long-run proportion of time spent in each state.
...
- **Taxi cab driver (Exercise 1.6):** A driver moves between locations according to a Markov chain; computing multi-step probabilities and long-run behavior.

## Key Takeaways

1. Matrix exponentiation is the computational engine for multi-step Markov chain analysis.

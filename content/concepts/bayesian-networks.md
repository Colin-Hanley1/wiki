...
This reduces storage from O(2^n) to O(n 2^m), where m is the maximum number of parents (indegree) of any node.

## Inference

- **Exact inference** — computing P(Q|E=e) — is #P-hard in general (Source: [[Quantum Inference on Bayesian Networks]]).
- **Approximate inference** via rejection sampling takes O(nm P(e)^{-1}) time classically, where P(e) is the marginal probability of the evidence. This becomes expensive when evidence is unlikely.
- Quantum approaches achieve a square-root speedup: O(n 2^m P(e)^{-1/2}) (Source: [[Quantum Inference on Bayesian Networks]]).
